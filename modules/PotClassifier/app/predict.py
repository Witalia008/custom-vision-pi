...
def extract_and_resize(img, targetSize):
    """
    resize and crop when opencv is not available
...
def preprocess_image_opencv(image_pil):
    ...
    return cv2.resize(
        image[starty : starty + crop_size, startx : startx + crop_size], new_size, interpolation=cv2.INTER_LINEAR,
    )
...
def predict_image(image):
    ...
    image: input PIL image
    returns prediction response as a dictionary.
    To get predictions, use result['predictions'][i]['tagName'] and result['predictions'][i]['probability']
    """
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for p, label in zip(predictions, labels):
                ...
                if truncated_probablity > 1e-8:
                    result.append(
                        {"tagName": label, "probability": truncated_probablity, "tagId": "", "boundingBox": None}
                    )
