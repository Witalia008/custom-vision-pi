import logging
from typing import Union, NoReturn, BinaryIO
...
def get_blob_from_uri(blob_service: BlobServiceClient, blob_uri: str) -> (BlobClient, str, str):
    ...
    blob_url_props = urlparse(blob_uri)
    ...
    input_blob = blob_service.get_blob_client(container=blob_container_name, blob=blob_name)
...
async def main(event: func.EventGridEvent, context: func.Context) -> NoReturn:
    blob_uri = event.get_json()["url"]
    blob_service = BlobServiceClient.from_connection_string(os.getenv("stovedatastorage_STORAGE"))
    ...
    with tempfile.NamedTemporaryFile(mode="wb", suffix=".mp4") as input_file:
        ...
        with tempfile.TemporaryDirectory() as frames_folder:
            ...
            with tempfile.NamedTemporaryFile(mode="w+b", suffix=".zip") as output_file:
                ...
                output_blob_container_name = "extractedframes"
                output_blob_name = os.path.splitext(input_blob_name)[0] + ".zip"

                output_blob = blob_service.get_blob_client(container=output_blob_container_name, blob=output_blob_name)
